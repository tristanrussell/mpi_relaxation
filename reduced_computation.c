 ...
 */
int calculate(double **in, int height, int width, double accuracy, int loop, int startHeight)
{
    ...
    if (loop < startHeight && (width - 1 - loop) > startHeight + (height - 2)) {
    ...
    if (loop >= (width - 2) / 2) {
        ...
        for (int i = loop + 2; i < width - loop; ++i) {
...
int main(int argc, char **argv)
{
    ...
    } else if (myrank < rowsLeft) {
        start = 1 + (lineCount * myrank) + myrank;
        end = start + lineCount + 1;
    } else {
        start = 1 + (lineCount * myrank) + rowsLeft;
        end = start + lineCount;
    }
